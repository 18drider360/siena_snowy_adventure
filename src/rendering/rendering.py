...
def draw_debug_coordinates(screen, player, platforms, camera_x, camera_y):
    """Draw coordinate indicators for the player and nearby platforms for debugging"""
    try:
    ...
    player_screen_x = int(player.rect.x - camera_x)
    player_screen_y = int(player.rect.y - camera_y)

    # Player coordinates at their position
    player_coord_text = f"({int(player.rect.x)}, {int(player.rect.y)})"
    coord_surface = font.render(player_coord_text, True, (255, 255, 0))
    coord_bg = pygame.Surface((coord_surface.get_width() + 4, coord_surface.get_height() + 4))
    coord_bg.fill((0, 0, 0))
    coord_bg.set_alpha(180)

    # Position above player
    screen.blit(coord_bg, (player_screen_x - coord_surface.get_width() // 2, player_screen_y - 60))
    screen.blit(coord_surface, (player_screen_x - coord_surface.get_width() // 2 + 2, player_screen_y - 58))

    # Draw coordinates for nearby platforms (within camera view)
    for platform in platforms:
        # Check if platform is visible on screen
        platform_screen_x = platform.x - camera_x
        platform_screen_y = platform.y - camera_y

        if (-200 < platform_screen_x < S.WINDOW_WIDTH + 200 and
            -100 < platform_screen_y < S.WINDOW_HEIGHT + 100):

            # Platform coordinates
            plat_coord_text = f"({int(platform.x)}, {int(platform.y)})"
            plat_surface = font.render(plat_coord_text, True, (0, 255, 255))
            plat_bg = pygame.Surface((plat_surface.get_width() + 4, plat_surface.get_height() + 4))
            plat_bg.fill((0, 0, 0))
            plat_bg.set_alpha(150)

            # Position at top-left corner of platform
            screen.blit(plat_bg, (platform_screen_x, platform_screen_y - 20))
            screen.blit(plat_surface, (platform_screen_x + 2, platform_screen_y - 18))
