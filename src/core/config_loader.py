...
class GameConfig:
    ...
    def _validate_config(self, config):
        ...
        """
        required_sections = ['display', 'audio', 'player']
        for section in required_sections:
            if section not in config:
                raise ValueError(f"Missing required config section: '{section}'")

        # Validate display settings
        if 'fps' not in config['display']:
            raise ValueError("Missing required config: display.fps")

        # Validate audio settings
        if 'master_enabled' not in config['audio']:
            raise ValueError("Missing required config: audio.master_enabled")

        # Validate player settings
        required_player_keys = ['speed', 'gravity', 'jump_strength', 'max_health']
        for key in required_player_keys:
            if key not in config['player']:
                raise ValueError(f"Missing required config: player.{key}")
    ...
    def _get_default_config(self):
        ...
        return {
            ...
            'player': {'speed': 4, 'gravity': 0.6, 'jump_strength': -13, 'max_health': 6},
