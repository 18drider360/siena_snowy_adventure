...
class GameProgression:
    ...
    COIN_REQUIREMENTS = {
        "Easy": {
            ...
            2: 19,   # Level 2: 54% of 35 = 19 coins
            3: 22,   # Level 3: 55% of 40 = 22 coins
            4: 25    # Level 4: 56% of 45 = 25 coins
        },
        "Medium": {
            1: 21,   # Level 1: 70% of 30 = 21 coins
            2: 26,   # Level 2: 74% of 35 = 26 coins
            3: 30,   # Level 3: 75% of 40 = 30 coins
            4: 31    # Level 4: 69% of 45 = 31 coins
        },
        "Hard": {
            1: 28,   # Level 1: 93% of 30 = 28 coins
            2: 32,   # Level 2: 91% of 35 = 32 coins
            3: 36,   # Level 3: 90% of 40 = 36 coins
            4: 40    # Level 4: 89% of 45 = 40 coins
        }
    }
    ...
    def __init__(self, difficulty="Medium", username="Player"):
        ...
        # --- CHECKPOINT SETTING ---
        self.checkpoints_enabled = False  # Can be toggled in settings
    ...
    def complete_level(self, level_num, coins_collected, time_taken, deaths):
        ...
        # Unlock next level (cap at MAX_LEVEL since that is the final level)
        if level_num >= self.max_level_reached:
            self.max_level_reached = min(level_num + 1, LevelManager.MAX_LEVEL)
...
class LevelManager:
    ...
    # Maximum number of levels in the game (Level 4 is the final level)
    MAX_LEVEL = 4

    # Checkpoint positions for each level (X coordinates)
    # Level 1: 1 checkpoint, Level 2: 2 checkpoints, Level 3: 2 checkpoints, Level 4: 3 checkpoints
    CHECKPOINTS = {
        1: [2850],  # Level 1: 1 checkpoint at x=2850 (on platform10 after hazard0, at y=520)
        2: [3100, 6000],  # Level 2: 2 checkpoints
        3: [3500, 7300],  # Level 3: 2 checkpoints
        4: [4500, 9500, 15200]  # Level 4: 3 checkpoints
    }

    # Ground Y positions for each level (for checkpoint flag placement)
    GROUND_Y = {
        1: 570,  # Level 1: ground at Y=570
        2: 400,  # Level 2: ground at Y=400
        3: 400,  # Level 3: ground at Y=400
        4: 400   # Level 4: ground at Y=400
    }
    ...
    LEVELS = {
        1: {
            'name': 'Winter Welcome',
            'module': 'src.levels.level_1_cabin',
            'world': '1-1'
        },
        2: {
            'name': 'Snow Cabin',
            'module': 'src.levels.level_2_ski_lift',
            'world': '1-2'
        },
        3: {
            'name': 'Mountain Climb',
            'module': 'src.levels.level_3_mountain_climb',
            'world': '1-3'
        },
        4: {
            'name': 'Northern Lights',
            'module': 'src.levels.level_4_northern_lights',
            'world': '1-4'
        }
    }
    ...
    @staticmethod
    def load_level_data(level_num, progression):
        """
        Load a level and return a LevelData object (new preferred method)
        ...
        Returns:
            LevelData: Level data object

        This is the new preferred method that returns a clean LevelData object
        instead of a 14-item tuple. Use this for new code.
        """
        from src.core.level_data import LevelData

        # Call the old load_level method to get the tuple
        result = LevelManager.load_level(level_num, progression)

        # Unpack the tuple
        (bg_color, platforms, hazards, level_width, player,
         enemies, projectiles, coins, world_name, goal_npc, background_layers,
         moving_platforms, disappearing_platforms, appearing_platforms) = result

        # Create and return LevelData object
        level_data = LevelData(
            ...
            disappearing_platforms=disappearing_platforms,
            appearing_platforms=appearing_platforms,
            goal_npc=goal_npc
        )

        logger.info(f"Loaded {level_data}")
        return level_data
